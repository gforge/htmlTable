...
#' @rdname htmlTable
#' @param view If you are using RStudio there is a viewer thar can render
...
print.htmlTable<- function (x, ..., view = interactive()) {
  if (getOption("htmlTable.cat", FALSE)) {
    return(cat(x))
  }
  ...
  html_print(HTML(x), viewer = if (view)
    viewerFunc)
  invisible(x)
}
