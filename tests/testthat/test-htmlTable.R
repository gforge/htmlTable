...
test_that("Test cell styles",{
  ...
  expect_equal(dim(prPrepareCellStyles(mx, "")),
               dim(mx))
  expect_equal(dim(prPrepareCellStyles(mx, "", header = mx_head, rnames = mx_rnames)),
               dim(mx))

  expect_equal(dim(prPrepareCellStyles(mx, "", header = mx_head, rnames = mx_rnames)),
               dim(mx))

  expect_equal(dim(prPrepareCellStyles(mx, rep("", times=ncol(mx)))),
               dim(mx))

  expect_error(prPrepareCellStyles(mx, rep("", times=nrow(mx))))


  mx_cell.style <- matrix(c("a", "b", "c", "d"), nrow=2, ncol=4, byrow = TRUE)
  expect_equal(prPrepareCellStyles(mx, mx_cell.style, rnames = mx_rnames)[2,1],
               "b")

  expect_error(prPrepareCellStyles(mx, mx_cell.style))

  mx_cell.style <- matrix(c("a", "b", "c", "d"), nrow=3, ncol=4, byrow = TRUE)
  expect_equal(prPrepareCellStyles(mx, mx_cell.style,
                                     header = mx_head,
                                     rnames = mx_rnames)[2,1],
               "b")

  expect_error(prPrepareCellStyles(mx, mx_cell.style, rnames = mx_rnames))
})